...
table_TRIRS_PS_indicador <- table_TRIRS_PS %>%
  ...
  dplyr::left_join(PIB_RSVA %>% 
                     dplyr::mutate(tri = base::factor(tri, levels = c("1", "2", "3", "4"))), by = c("atividade" = "atividade",
                                    "Ano" = "ano",
                                    "Trimestre" = "tri")) %>% 
  dplyr::group_by(atividade, Ano, Trimestre) %>% 
  dplyr::mutate(VA_RS =  gsub(",", ".", VA_RS),
                VA_RS = base::as.numeric(VA_RS),
                indicadorVA_N = VA_RS/soma_N,
                indicadorVA_qtd_HHabituais = VA_RS/qtd_horasHabituais,
                indicadorVA_qtd_HEfetivas = VA_RS/qtd_horasEfetivas)
  ...
  dplyr::left_join(PIB_BRVA %>% 
                     dplyr::mutate(tri = base::factor(tri, levels = c("1", "2", "3", "4")),
                                   ...
                                   by = c("atividade" = "atividade",
                                          ...
                                          "Trimestre" = "tri")) %>% 
  dplyr::group_by(atividade, Ano, Trimestre) %>% 
  dplyr::mutate(VA_BR =  gsub(",", ".", VA_BR),
                VA_BR = base::as.numeric(VA_BR),
                indicadorVA_N = VA_BR/soma_N,
                indicadorVA_qtd_HHabituais = VA_BR/qtd_horasHabituais,
                indicadorVA_qtd_HEfetivas = VA_BR/qtd_horasEfetivas)
...
table_ANORS_PS_indicador <- table_ANORS_PS %>%
  ...
  dplyr::mutate(indicadorVA_N = VA_RS_soma/soma_N,
                indicadorVA_qtd_HHabituais = VA_RS_soma/qtd_horasHabituais,
                indicadorVA_qtd_HEfetivas = VA_RS_soma/qtd_horasEfetivas)
...
table_ANOBR_PS_indicador <- table_ANOBR_PS %>%
  ...
  dplyr::mutate(indicadorVA_N = VA_BR_soma/soma_N,
                indicadorVA_qtd_HHabituais = VA_BR_soma/qtd_horasHabituais,
                indicadorVA_qtd_HEfetivas = VA_BR_soma/qtd_horasEfetivas) %>% 
  dplyr::distinct()
...
writexl::write_xlsx(sheets, 
           paste0("C:/Users/fernanda-romeiro/OneDrive - Governo do Estado do Rio Grande do Sul/Projetos/PNAD/PNAD_projetos/Dados/table_PS_3.xlsx"))
```
