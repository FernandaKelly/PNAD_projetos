...
```{r}
dadosPNADc1VISITA_20_22 <- visita5_pnad %>% 
...
```{r}
#############################
dadosPNADC_ANOS_BR <- dadosPNADc1VISITA_12_15 %>% 
  dplyr::bind_rows(dadosPNADc1VISITA_16_19) 
  
dadosPNADC_ANOS_BR <- dadosPNADC_ANOS_BR %>% 
  dplyr::bind_rows(dadosPNADc1VISITA_20_22) 

########################3

dadosPNADC_ANOS_BR <- dadosPNADC_ANOS_BR %>% 
  dplyr::bind_rows(dadosPNADc1VISITA_23_24)
```

```{r}
save("C:/Users/fernanda-romeiro/OneDrive - Governo do Estado do Rio Grande do Sul/Projetos/PNAD/Dados_completos/dadosPNADc2023_completo.RData")
```

```{r}
load(file = "C:/Users/fernanda-romeiro/OneDrive - Governo do Estado do Rio Grande do Sul/Projetos/PNAD/PNAD_projetos/Dados/dadosPNADC_ANOS_BR.RData")
```
...
```{r}
dadosPNADC_ANOS_BRPR <- dadosPNADC_ANOS_BR %>% 
  dplyr::filter(!(is.na(V4013))) %>%
  dplyr::filter(VD4002 == "Pessoas ocupadas") 

dadosPNADC_ANOS_BRPR  <- PNADcIBGE::pnadc_design(dadosPNADC_ANOS_BRPR)
dadosPNADC_ANOS_BRSRPR <- srvyr::as_survey(dadosPNADC_ANOS_BRPR)
```

### Total de Pessoas

```{r}
table_1P <- dadosPNADC_ANOS_BRSRPR %>% 
...
table_2P <- dadosPNADC_ANOS_BRSRPR %>%  
  dplyr::group_by(cod_SCN_PR, Ano,Trimestre) %>% 
...
table_3P <- dadosPNADC_ANOS_BRSRPR %>%
...
dadosPNADC_ANOS_BRSEC <-  dadosPNADC_ANOS_BR %>% 
  dplyr::filter(!(is.na(V4044))) %>% 
  dplyr::filter(VD4002 == "Pessoas ocupadas")  
  
dadosPNADC_ANOS_BRSEC  <- PNADcIBGE::pnadc_design(dadosPNADC_ANOS_BRSEC)
dadosPNADC_ANOS_BRSRSEC <- srvyr::as_survey(dadosPNADC_ANOS_BRSEC)
```

### Total de Pessoas

```{r}
table_1S <- dadosPNADC_ANOS_BRSRSEC %>% 
...
table_2S <- dadosPNADC_ANOS_BRSRSEC %>% 
  dplyr::group_by(cod_SCN_SEC, Ano,Trimestre) %>% 
...
table_3S <- dadosPNADC_ANOS_BRSRSEC %>%  
...
```{r}
table_1TP <- dadosPNADC_ANOS_BRSRPR %>% 
...
table_2TP <- dadosPNADC_ANOS_BRSRPR %>%  
  dplyr::group_by(Ano,Trimestre) %>% 
  dplyr::summarise(Qtd_horasHabituais = srvyr::survey_total(V4039,
                                                   na.rm = TRUE,
                                                 vartype = c("se", "ci", "var", "cv")))
...
```{r}
table_3TP <- dadosPNADC_ANOS_BRSRPR %>%
  dplyr::filter(V4039C != 0) %>% 
  dplyr::group_by(Ano,Trimestre) %>% 
  dplyr::summarise(Qtd_horasEfetivas = srvyr::survey_total(V4039C,
                                                   na.rm = TRUE,
...
```{r}
table_1TS <- dadosPNADC_ANOS_BRSRSEC %>% 
  dplyr::group_by(Ano,Trimestre) %>% 
  dplyr::summarise(freq = srvyr::survey_total(vartype = c("se", "ci", "var", "cv")))
```
...
table_2TS <- dadosPNADC_ANOS_BRSRSEC %>% 
  dplyr::group_by(Ano,Trimestre) %>% 
  dplyr::summarise(Qtd_horasHabituais = srvyr::survey_total(V4056,
                                                   na.rm = TRUE,
                                                   vartype = c("se", "ci", "var", "cv")))
...
table_3TS <- dadosPNADC_ANOS_BRSRSEC %>%  
  dplyr::filter(V4056C != 0) %>% 
  dplyr::group_by(Ano,Trimestre) %>% 
  dplyr::summarise(Qtd_horasEfetivas = srvyr::survey_total(V4056C,
                                                   na.rm = TRUE,
...
dadosPNADc_PS_1 <- dadosPNADC_ANOS_BR %>%
  #dplyr::filter(!(is.na(V4044))) %>% 
  dplyr::filter(VD4002 == "Pessoas ocupadas") %>% 
...
dadosPNADc_PS_2 <- dadosPNADC_ANOS_BR %>% 
...
dadosPNADc_PS_3 <- dadosPNADC_ANOS_BR %>%
  dplyr::filter(VD4002 == "Pessoas ocupadas") %>%
  tidyr::pivot_longer(cols = c(cod_SCN_PR, cod_SCN_SEC), 
                      names_to = "tipo",
                      values_to = "atividade") %>%
...
dadosPNADc_PS_4 <- dadosPNADC_ANOS_BR %>% 
...
dadosPNADc_PS_5 <- dadosPNADC_ANOS_BR %>%
  dplyr::filter(VD4002 == "Pessoas ocupadas") %>%
  tidyr::pivot_longer(cols = c(cod_SCN_PR, cod_SCN_SEC), 
                      names_to = "tipo",
                      values_to = "atividade") %>%
...
dadosPNADc_PS_6 <- dadosPNADC_ANOS_BR %>% 
...
writexl::write_xlsx(sheets, 
           paste0("C:/Users/fernanda-romeiro/OneDrive - Governo do Estado do Rio Grande do Sul/Projetos/PNAD/PNAD_projetos/Dados/tabPSANOS_BR_1.xlsx"))
```
