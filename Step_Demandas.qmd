...
dadosPNADc1VISITA_2013$variables$V1030
dadosPNADc1VISITA_2014$variables$V1030
dadosPNADc1VISITA_2015$variables$V1030
dadosPNADc1VISITA_2024$variables$V1030
```
...
table_12_pop <- dadosPNADc1VISITA_2012 %>% 
dplyr::group_by(UF, V1030) %>%
    dplyr::summarise(
      freq_1030 = srvyr::survey_total(V1030, vartype = "cv"),
      freq_1034 = srvyr::survey_total(V1034, vartype = "cv"))


table_12_pop <- dadosPNADc1VISITA_2012 %>% 
dplyr::group_by(UF, V2009) %>%
    dplyr::summarise(
      freq = srvyr::survey_total(vatype = "cv")) %>% 
  dplyr::group_by(UF) %>% 
...
table_1 <- imap_dfr(lista_dados, function(dados, ano) {
  
  desenho <- PNADcIBGE::pnadc_design(dados)
  survey  <- srvyr::as_survey(desenho)
...
table_2 <- imap_dfr(lista_dados, function(dados, ano) {
...
table_3 <- imap_dfr(lista_dados, function(dados, ano) {
  
  desenho <- PNADcIBGE::pnadc_design(dados)
  survey  <- srvyr::as_survey(desenho)
  
  survey %>%
    filter(VD4001 == "Pessoas fora da força de trabalho") %>% 
    group_by(UF) %>%
    summarise(
      freq_1030 = srvyr::survey_total(V1030, vartype = "cv"),
      freq_1034 = srvyr::survey_total(V1034, vartype = "cv"),
...
table_4 <- imap_dfr(lista_dados, function(dados, ano) {
...
table_5 <- imap_dfr(lista_dados, function(dados, ano) {
